...
# set up a jet, 以topLeft为原点, 向右为+x, 向下为+y, (x, y, xSpan, ySpan)
player_rect = []
...
while running:
	...
	if not player.is_hit:
		...
		# if shoot_frequency % god_mode == 0: 
			bullet_sound.play()
			# 累计15帧的时间, 进行一次子弹发射, 即1/4秒发射一次
			if mode_awesome is True:
	...
	# 生成敌机
	if enemy_frequency % enemy_cnt == 0:
		# 每50帧刷新一架敌机, 即5/6秒出现一架新的敌机, 可用于调整敌机数量
		# 初始位置: x为随机, y为0
